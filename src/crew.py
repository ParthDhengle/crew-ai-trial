...
class AiAgent:
    ...
    def run_workflow(self, user_query: str):
        ...
        try:
            available_operations = json.loads(available_operations_content.strip()).get("operations", [])  # FIX: Extract "operations" list
        ...
        # Load user profile from Firestore (unchanged)
        ...
        # 3. Knowledge Retrieval (unchanged)
        kb_context = {}
        ...
        # 5. Save history (unchanged)
        history.append({"role": "user", "content": user_query})
        history.append({"role": "assistant", "content": final_response})
        ChatHistory.save_history(history)

        # --- OPTIMIZATION: Memory extraction (unchanged) ---
    ...
    def perform_operations(self, plan: dict) -> str:
        ...
        # Also handle params key mismatch if any
