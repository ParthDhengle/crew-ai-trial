...
class AiAgent():
    ...
    def run_workflow(self, user_query: str):
        ...
        user_profile_raw = file_tool._run(inputs['user_profile_path']).split('Content of ', 1)[-1]
        ...
        op_data = json.loads(available_operations)
        op_names = "\n".join([op['name'] for op in op_data])
        ...
        # --- Preprocessor: Load/assemble memory ---
        ...
        # --- Periodic narrative ---
        if len(history) % self.memory_manager.policy['narrative_interval'] == 0:
            ...
            summaries = self.memory_manager.load_json(summaries_path)
