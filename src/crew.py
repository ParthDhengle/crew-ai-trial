...
from firebase_client import add_task, get_tasks  # Integrate Firebase for tasks
...
class AiAgent():
    ...
    def __init__(self):
        ...
        self.memory_manager = MemoryManager()  # Updated to use Firebase + FAISS
        super().__init__()

    # === Agents (unchanged except memory_extractor uses planner_llm) ===
    @agent
    ...
    # === Tasks (updated prompts to mention Firebase/FAISS where relevant) ===
    @task
    ...
    def plan_execution(self) -> Task:
        return Task(config=self.tasks_config['plan_execution'])  # Updated in YAML
    ...
    # === Helpers (unchanged except integrate Firebase in perform_operations if needed) ===
    ...
    def run_workflow(self, user_query: str):
        ...
        # Read user profile and operations (unchanged)
        ...
        user_profile = json.loads(user_profile_raw_content.strip()) if user_profile_raw_content.strip() else {}
        ...
        try:
            ...
            classification = {'mode': 'agentic', 'required_operations': [op['name'] for op in available_operations], 'required_kb': ['short_term', 'long_term', 'narrative']}

        # Knowledge Retrieval (updated to use MemoryManager with Firebase)
        kb_context = {}
        ...
        if 'long_term' in required_kb and len(user_query.split()) > 3:
            kb_context['long_term'] = self.memory_manager.retrieve_long_term(user_query)  # Uses FAISS + Firebase fallback
        if 'narrative' in required_kb:
            kb_context['narrative'] = self.memory_manager.get_narrative_summary()  # From Firebase summaries
        ...
        if classification['mode'] == 'direct':
            ...
            if 'error' in plan:
                ...
                # Executor: Use OperationsTool (now Firebase-integrated)
                try:
        ...
        # Memory extraction (updated to use MemoryManager.update_long_term)
        ...
        if should_extract_memory:
            try:
                ...
                self.memory_manager.update_long_term(extracted)  # Now saves to Firebase + local
            except Exception as e:
                print(f"Memory extraction failed: {e}")

        # Periodic narrative (updated)
    ...
    def perform_operations(self, plan: dict) -> str:
        ...
        # Remap keys if needed (unchanged)
        ...
        ops_tool = OperationsTool()  # Now Firebase-integrated
