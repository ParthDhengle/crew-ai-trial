...
# Resolve project root and load .env from there, overriding any system/user env vars
...
Consider setting it in {ENV_PATH} or placing NOVA_firebase_credentials.json in the project root."""
        )
...
def update_document(collection: str, doc_id: str, data: dict, subcollection: bool = True) -> bool:
    ...
    try:
        if subcollection:
            get_user_ref().collection(collection).document(doc_id).update(data)
        else:
...
def query_collection(collection: str, filters: list = None, limit: int = None, subcollection: bool = True) -> list:
    ...
    if filters:
        # filters is expected as list of tuples: (field, op, value)
        for field, op, value in filters:
            # Use FieldFilter for complex queries if available; fallback to where()
            try:
...
def delete_document(collection: str, doc_id: str, subcollection: bool = True) -> bool:
    ...
    try:
        if subcollection:
            get_user_ref().collection(collection).document(doc_id).delete()
        else:
...
def add_project(name: str, description: str = None, members: list = None) -> str:
    """Create project."""
    data = {"name": name, "description": description, "owner_id": USER_ID, "members": members or [],
            "created_at": datetime.now().isoformat()}
...
def end_focus_session(session_id: str) -> bool:
    """End focus session."""
    return update_document("focus_sessions", session_id,
...
def create_snapshot(paths: list, retention_days: int = 30) -> str:
    ...
    snap_id = add_document("snapshots", {
        ...
        "object_store_uri": f"snapshots/{USER_ID}/{snap_id}",
    })
...
def add_notification(type_: str, message: str) -> str:
    """Add notification."""
    data = {"type": type_, "message": message, "status": "pending", "created_at": datetime.now().isoformat()}
...
def add_expense(file_ref: str, amount: float, currency: str, category: str, date: str, vendor: str) -> str:
    """Add expense."""
    data = {"file_ref": file_ref, "amount": amount, "currency": currency, "category": category, "date": date, "vendor": vendor}
...
def add_kb_entry(title: str, content_md: str, tags: list = None, references: list = None) -> str:
    ...
    data = {"title": title, "content_md": content_md, "tags": tags or [], "references": references or [], "created_at": datetime.now().isoformat()}
...
def add_summary(date_: str, summary_text: str, metrics: dict = None) -> str:
    ...
    data = {"date": date_, "summary_text": summary_text, "metrics": metrics or {}, "created_at": datetime.now().isoformat()}
...
def add_rule(trigger_type: str, conditions: dict, actions: list, enabled: bool = True) -> str:
    """Add automation rule."""
    data = {"trigger_type": trigger_type, "conditions": conditions, "actions": actions, "enabled": enabled, "created_at": datetime.now().isoformat()}
...
def queue_operation(op_name: str, params: dict) -> str:
    ...
    data = {"op_name": op_name, "params": params, "status": "pending", "created_at": datetime.now().isoformat()}
